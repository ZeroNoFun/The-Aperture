...
if CLIENT then
	...
	language.Add("tool.aperture_crusher.desc", "Crusher that can change")
	...
	language.Add("tool.aperture_crusher.startenabled.help", "Crusher will be activated when placed")
	language.Add("tool.aperture_crusher.twoside", "Two Sided")
	language.Add("tool.aperture_crusher.twoside.help", "Spawning crusher on each side")
	language.Add("tool.aperture_crusher.countx", "Copy Back")
	language.Add("tool.aperture_crusher.county", "Copy Right")
...
local function GetSpawnPos(pos, ang, ply, normal)
	...
	if crusherPos == gridPos then return 1 end
	return 0
end
...
if SERVER then
	duplicator.RegisterEntityClass("ent_portal_crusher", MakeCrusher, "pos", "ang", "length", "key_enable", "startenabled", "toggle", "data")
end
...
function TOOL:LeftClick(trace)
	...
	for y=0,county - 1 do
		if twoside == 1 then
			...
			table.insert(crusherPanelTable, ent)
		end
	end
	end
		
	undo.Create("Crusher")
		for k,v in pairs(crusherPanelTable) do undo.AddEntity(v) end
		undo.SetPlayer(ply)
	undo.Finish()
...
function TOOL.BuildCPanel( CPanel )
	...
	CPanel:AddControl("CheckBox", {Label = "#tool.aperture_crusher.twoside", Command = "aperture_crusher_twoside", Help = true})
	CPanel:AddControl("CheckBox", {Label = "#tool.aperture_crusher.startenabled", Command = "aperture_crusher_startenabled", Help = true})
	CPanel:AddControl("Numpad", {Label = "#tool.aperture_crusher.enable", Command = "aperture_crusher_keyenable"})
	CPanel:AddControl("CheckBox", {Label = "#tool.aperture_crusher.toggle", Command = "aperture_crusher_toggle"})
end